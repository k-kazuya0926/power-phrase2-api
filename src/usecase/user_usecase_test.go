...
// 入力用ユーザー
func makeMockUserForInput(id int) *model.User {
	user := &model.User{
...
// DBから取得されたユーザー
func makeMockUserForRead(id int) *model.User {
	user := makeMockUserForInput(id)
	user.ID = id
...
func TestLogin_success(t *testing.T) {
	...
	usecase := NewUserUseCase(&repository)
	id := 1
	userForInput := makeMockUserForInput(id)
	userForRead := makeMockUserForRead(id)
...
func TestGetUser_success(t *testing.T) {
	...
	usecase := NewUserUseCase(&repository)
	id := 1
	expected := makeMockUserForRead(id)
	expected.Password = ""
